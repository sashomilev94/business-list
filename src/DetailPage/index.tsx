...
function DetailPage(props: { businessData: BusinessData[] }) {
	...
	const singleBusiness = businessData.find((d) => d.id === id);
	const nearbyBusiness = businessData.filter((d) => d.address.city === singleBusiness?.address.city && d.id !== id);
					...
					<div className="DetailsInfoCol">
						<div className="DetailsInfoPlaces">
							...
							{nearbyBusiness.map(nb => {
								return <NearbyPlace key={nb.id} name={nb.name} address={nb.address} />
							})}
						</div>
					</div>
				</div>
